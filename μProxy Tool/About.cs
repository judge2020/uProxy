...
namespace μProxy_Tool
{
	...
	public partial class About : Form
	{
		...
		{
			// Token: 0x0600012E RID: 302 RVA: 0x00176454 File Offset: 0x00174854
			[CompilerGenerated]
			public void _Lambda$__1()
			{
				this.$VB$Me.PictureBox2.Hide();
				if (Conversions.ToDouble(this.$VB$Local_str.Split(new char[]
				{
					','
				})[0]) > Conversions.ToDouble(this.$VB$Me.ProductVersion))
				{
					this.$VB$Me.download_link = this.$VB$Local_str.Split(new char[]
					{
						','
					})[1];
					this.$VB$Me.LinkLabel1.Text = "Download μProxy Tool " + this.$VB$Local_str.Split(new char[]
					{
						','
					})[0];
					this.$VB$Me.LinkLabel1.Show();
				}
				else
				{
					this.$VB$Me.Version_Lbl.Text = "Version: " + this.$VB$Me.ProductVersion + " (Latest)";
				}
			}

			// Token: 0x0400007B RID: 123
			public string $VB$Local_str;

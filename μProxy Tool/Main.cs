...
namespace μProxy_Tool
{
	...
	public partial class Main : Form
	{
		...
		public void Startchecker()
		{
			...
			try
			{
				HashSet<string>.Enumerator enumerator = this.proxies.GetEnumerator();
		...
		private void UpdateStatus(string status, bool isscraper)
		{
			...
			catch (ObjectDisposedException expr_31)
			{
				ProjectData.SetProjectError(expr_31);
				ProjectData.ClearProjectError();
			}
			catch (NullReferenceException expr_3F)
			{
		...
		private void Addlistview(string proxy, string status, string country, string passed, int timeout)
		{
			...
			catch (ObjectDisposedException expr_4D)
			{
				ProjectData.SetProjectError(expr_4D);
				ProjectData.ClearProjectError();
			}
			catch (NullReferenceException expr_5B)
			{
		...
		public List<string> Loadlist()
		{
			...
			try
			{
				...
				if (MySettingsProperty.Settings.Inbuild_Source)
				{
					try
					{
						IEnumerator enumerator = new Regex(pattern, RegexOptions.IgnoreCase | RegexOptions.Singleline).Matches(input).GetEnumerator();
						while (enumerator.MoveNext())
						{
			...
			checked
			{
				try
				{
					...
					for (int i = 0; i < array.Length; i++)
					{
						...
						if (text.Contains("{") && text.Contains("}"))
						{
							...
							int arg_18F_0 = Conversions.ToInteger(match2.Groups[1].Value);
							int num2 = Conversions.ToInteger(match2.Groups[2].Value);
							int num3 = num;
							int num4 = arg_18F_0;
							while ((num3 >> 31 ^ num4) <= (num3 >> 31 ^ num2))
							{
								if (match2.Groups[1].Value.Contains("0"))
								{
									if (num4 <= 9)
									{
										...
										{
											'{'
										})[0] + "0" + Conversions.ToString(num4) + text.Split(new char[]
										{
											'}'
										})[1]);
									}
									else
									{
										...
										{
											'{'
										})[0] + Conversions.ToString(num4) + text.Split(new char[]
										{
											'}'
										})[1]);
									}
				...
				catch (Exception expr_285)
				{
					ProjectData.SetProjectError(expr_285);
					ProjectData.ClearProjectError();
				}
				return list;
			}
		...
		public void startscraper()
		{
			...
			try
			{
				List<string>.Enumerator enumerator = this.proxy_source.GetEnumerator();
				while (enumerator.MoveNext())
				{
					string current = enumerator.Current;
					ThreadPool.QueueUserWorkItem(new WaitCallback(this.scraper), current);
				}
			}
			finally
			{
		...
		public void scraper(object psource)
		{
			try
			{
				...
				try
				{
					...
					{
						string current = enumerator.Current;
						if (Operators.CompareString(current, "", true) != 0)
						{
							this.proxies.Add(current);
						}
					}
				}
				finally
			...
			checked
			{
				...
				if (saveFileDialog.ShowDialog() == DialogResult.OK)
				{
					using (StreamWriter streamWriter = new StreamWriter(saveFileDialog.FileName, true))
					{
						try
						{
							HashSet<string>.Enumerator enumerator = s.GetEnumerator();
							while (enumerator.MoveNext())
							{
								string current = enumerator.Current;
		...
		// Token: 0x0600010C RID: 268 RVA: 0x00176540 File Offset: 0x00174940
		...
		// Token: 0x0600010D RID: 269 RVA: 0x00176550 File Offset: 0x00174950
		[CompilerGenerated]
		...
		// Token: 0x0600010E RID: 270 RVA: 0x001765A8 File Offset: 0x001749A8
		[CompilerGenerated]
		...
		// Token: 0x0600010F RID: 271 RVA: 0x00176890 File Offset: 0x00174C90
		[CompilerGenerated]
		private void _Lambda$__7()
		{
			this.enable_btn();
			if (MySettingsProperty.Settings.Auto_Save)
			{
		...
		// Token: 0x06000110 RID: 272 RVA: 0x00176A80 File Offset: 0x00174E80
		[CompilerGenerated]
		...
		[OptionText, CompilerGenerated]
		internal class _Closure$__2
		{
			// Token: 0x06000130 RID: 304 RVA: 0x001765D8 File Offset: 0x001749D8
			[CompilerGenerated]
			public void _Lambda$__5()
			{
				this.$VB$Me.status_lbl.Text = this.$VB$Local_status;
				if (this.$VB$Local_isscraper)
				{
					this.$VB$Me.status_progress.Value = checked((int)Math.Round(Conversion.Val(unchecked((double)this.$VB$Me.@checked / (double)this.$VB$Me.proxy_source.Count * 100.0))));
					this.$VB$Me.status_per.Text = Conversions.ToString(Convert.ToInt32((double)this.$VB$Me.@checked / (double)this.$VB$Me.proxy_source.Count * 100.0)) + "%";
				}
				else
				{
					this.$VB$Me.status_progress.Value = checked((int)Math.Round(Conversion.Val(unchecked((double)this.$VB$Me.@checked / (double)this.$VB$Me.proxies.Count * 100.0))));
					this.$VB$Me.status_per.Text = Conversions.ToString(Convert.ToInt32((double)this.$VB$Me.@checked / (double)this.$VB$Me.proxies.Count * 100.0)) + " %";
				}
			}

			// Token: 0x0400007D RID: 125
			public bool $VB$Local_isscraper;

			// Token: 0x0400007E RID: 126
			public string $VB$Local_status;
		...
		{
			// Token: 0x06000132 RID: 306 RVA: 0x0017673C File Offset: 0x00174B3C
			[CompilerGenerated]
			public void _Lambda$__6()
			{
				ListViewItem listViewItem = this.$VB$Me.ListView1.Items.Add(Conversions.ToString(checked(this.$VB$Me.ListView1.Items.Count + 1)));
				listViewItem.UseItemStyleForSubItems = false;
				listViewItem.SubItems.Add(this.$VB$Local_proxy);
				listViewItem.SubItems.Add(this.$VB$Local_country).ForeColor = ColorTranslator.FromHtml("#266cd4");
				this.$VB$Me.GetData.SetAnonColor(this.$VB$Local_status);
				listViewItem.SubItems.Add(this.$VB$Local_status).BackColor = this.$VB$Me.GetData.Color;
				listViewItem.SubItems.Add(this.$VB$Local_passed.ToString()).ForeColor = ((Operators.CompareString(this.$VB$Local_passed, "True", true) == 0) ? Color.Green : Color.Red);
				this.$VB$Me.GetData.SetTColor(this.$VB$Local_timeout);
				listViewItem.SubItems.Add(Conversions.ToString(this.$VB$Local_timeout)).ForeColor = this.$VB$Me.GetData.Color;
				listViewItem.SubItems[3].ForeColor = Color.White;
			}

			// Token: 0x04000080 RID: 128
			public string $VB$Local_country;

			// Token: 0x04000081 RID: 129
			public string $VB$Local_passed;

			// Token: 0x04000082 RID: 130
			public string $VB$Local_proxy;

			// Token: 0x04000083 RID: 131
			public string $VB$Local_status;

			// Token: 0x04000084 RID: 132
			public int $VB$Local_timeout;
		...
		{
			// Token: 0x06000134 RID: 308 RVA: 0x001768E0 File Offset: 0x00174CE0
			[CompilerGenerated]
			public void _Lambda$__8()
			{
				try
				{
					...
					StreamWriter streamWriter = MyProject.Computer.FileSystem.OpenTextFileWriter(Application.StartupPath + "\\Data\\Proxies\\" + this.$VB$Local_choice + ".txt", false);
					try
					{
						IEnumerator enumerator = this.$VB$Me.ListView1.Items.GetEnumerator();
						while (enumerator.MoveNext())
						{
							ListViewItem listViewItem = (ListViewItem)enumerator.Current;
							if (Operators.CompareString(this.$VB$Local_choice, "All_Checked", true) == 0)
							{
								streamWriter.WriteLine(listViewItem.SubItems[1].Text);
							}
							else if (Operators.CompareString(listViewItem.SubItems[3].Text, this.$VB$Local_choice, true) == 0)
							{
								streamWriter.WriteLine(listViewItem.SubItems[1].Text);
							}
						}
